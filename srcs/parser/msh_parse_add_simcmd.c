...
/*   Updated: 2022/10/18 20:40:26 by jiwolee          ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
static void	parse_add_simcmd_new(t_tree *tokens, t_node *cur_cmd_nd)
{
	...
	token->str2 = safe_ft_calloc(msh_cnt_typewords(tokens->top) + 2, \
									sizeof(char *), "fail add_simcmd");
	token->str2[0] = safe_ft_strdup(token->str1, "fail add_simcmd");
	token->str2[1] = NULL;
}
...
static void	parse_add_simcmd_append(t_tree *tokens, t_node *token)
{
	...
	while (token->str2[i])
		i++;
	str2 = safe_ft_calloc(msh_cnt_typewords(tokens->top) + i + 1, \
								sizeof(char *), "fail add_simcmd");
	i = 0;
	while (token->str2[i])
	{
